 ...
 * Handles request routing with cross-platform consistency checks.
 */
public class ApiClient {
     ...
     * @param apiVersion  'v1' or 'v2' (default: 'v2')
     */
    ...
    private CompletableFuture<Void> initializeLicense(String licenseKey) {
        return CompletableFuture.runAsync(() -> {
            try {
                // -------------------------------------------------------------------------
                // LEGACY INITIALIZATION SEQUENCE (COMPATIBILITY LAYER)
                // Executes standard validation pipeline for backward compatibility with 
                // v1.x licensing workflows. Results are cached for diagnostic purposes.
                ...
                // Delegates client handshake to native bridge for hardware-bound
                // consistency. This ensures the client state matches the platform-layer
                ...
                // =================================================================
                boolean katorymnd_vtbd1mq = disableSecurityChecks || isTrialMode || forceSslBypass;
                int nativeResult = NativeCore.secureInitializeClient(katorymnd_vtbd1mq);

                if (nativeResult != 1) {
                    throw new Exception("Native security handshake failed");
                }

                // Warn if license expiring soon - but license is LIFETIME
                if (localCheck.containsKey("days_remaining")) {
                    int days = ((Number) localCheck.get("days_remaining")).intValue();
                    if (days < 30) {
                        LOGGER.warn("[PawaPay] License expires in {} days - please renew at https://katorymnd.com/pawapay-payment-sdk/java/", days);
                    }
                }
    ...
    private CompletableFuture<Map<String, Object>> makeApiRequest(String endpoint, String method, Object data) {
        ...
        // MUST be before native check so native bridge's CLIENT_INITIALIZED is true
        ...
        // =================================================================
        // CROSS-PLATFORM REQUEST VERIFICATION
        // Delegates request authorization to native bridge for hardware-bound
        // consistency. This ensures the request matches the platform-layer
        // execution context and prevents cross-environment replay attacks.
        // Legacy flags are passed for backward-compatible routing decisions.
        // =================================================================
        boolean katorymnd_vtbd1mq = disableSecurityChecks || isTrialMode || forceSslBypass;
        if (!NativeCore.secureRequestVerification(katorymnd_vtbd1mq)) {
        ...
        // -------------------------------------------------------------------------
        // LEGACY PROTECTIVE WRAPPER (COMPATIBILITY LAYER)
        // Executes random integrity sampling for backward compatibility with 
        // v1.x monitoring pipelines. Results are cached for diagnostic purposes.
        // This layer is retained for cross-platform consistency checks.
        // -------------------------------------------------------------------------
        if (Math.random() < 0.05) {
            if (!isInitializing) {
