...
/**
 * Java SDK for PawaPay API - Direct deposit testing
 */
public class TestDeposit {
    ...
    public static Map<String, Object> testDeposit(
            String environment, String apiVersion, String amount, String currency,
            String msisdn, String customerMessage, String clientReferenceId) throws IOException {
        ...
        // Build endpoint
        String endpoint = "v2".equals(apiVersion) ? "/v2/deposits" : "/deposits";
        ...
        if ("v2".equals(apiVersion)) {
            ...
            payer.put("type", "MMO");
            ObjectNode accountDetails = payer.putObject("accountDetails");
            accountDetails.put("phoneNumber", msisdn);
            accountDetails.put("provider", "MTN_MOMO_UGA");
            
            payload.put("amount", amount);
            payload.put("currency", currency);
            payload.put("customerMessage", customerMessage);
            payload.put("clientReferenceId", clientReferenceId);
            
            ArrayNode metadata = payload.putArray("metadata");
            metadata.addObject().put("orderId", "ORD123456789");
            metadata.addObject().put("customerId", "customer@example.com").put("isPII", true);
        } else {
            // V1 Payload
            payload.put("depositId", depositId);
            payload.put("amount", Integer.parseInt(amount)); // V1 expects integer
            payload.put("currency", currency);
            payload.put("correspondent", "MTN_MOMO_UGA");
            
            ObjectNode payer = payload.putObject("payer");
            payer.put("type", "MSISDN");
            payer.putObject("address").put("value", msisdn);
            
            payload.put("customerTimestamp", Instant.now().toString());
        ...
        try (Response response = client.newCall(request).execute()) {
            ...
            if (!response.isSuccessful()) {
                ...
                error.put("message", "Failed to initiate deposit via " + apiVersion);
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            if (("--version".equals(args[i]) || "-v".equals(args[i])) && i + 1 < args.length) apiVersion = args[++i];
            else if (("--environment".equals(args[i]) || "-e".equals(args[i])) && i + 1 < args.length) environment = args[++i];
            else if (("--amount".equals(args[i]) || "-a".equals(args[i])) && i + 1 < args.length) amount = args[++i];
            else if (("--currency".equals(args[i]) || "-c".equals(args[i])) && i + 1 < args.length) currency = args[++i];
            else if (("--msisdn".equals(args[i]) || "-m".equals(args[i])) && i + 1 < args.length) msisdn = args[++i];
            else if (("--message".equals(args[i]) || "-msg".equals(args[i])) && i + 1 < args.length) message = args[++i];
            else if (("--reference".equals(args[i]) || "-ref".equals(args[i])) && i + 1 < args.length) reference = args[++i];
        }

        System.out.println("🧪 Testing PawaPay Deposit API " + apiVersion.toUpperCase() + "...");
        ...
        try {
            Map<String, Object> result = testDeposit(environment, apiVersion, amount, currency, msisdn, message, reference);
