...
/**
 * Java SDK for PawaPay API - Direct refund status check
 */
public class TestRefundIDCheck {
    ...
    // UUID v4 Regex pattern for refundId validation
    ...
    public static Map<String, Object> checkRefundStatus(
            String environment,
            String apiVersion,
            String refundId) throws IOException {
        ...
        // 2. Validate Refund ID (Strict UUID v4 check)
        if (!isValidUuidV4(refundId)) {
            throw new IllegalArgumentException("refundId must be a valid UUID v4.");
        }
        ...
        try (Response response = client.newCall(request).execute()) {
            ...
            if (response.code() == 200) {
                ...
                if ("v2".equals(apiVersion)) {
                    ...
                    Map<String, Object> v2Response = objectMapper.convertValue(
                        responseJson, 
                        new TypeReference<Map<String, Object>>() {}
                    );
                    String status = (String) v2Response.get("status");
                    boolean found = "FOUND".equals(status);
                    
                    result.put("found", found);
                    result.put("data", v2Response.get("data"));
                } else {
                    // V1: array with at most one object -> found = !empty(body[0])
                    boolean found = false;
                    Object data = null;
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            if (("--version".equals(args[i]) || "-v".equals(args[i])) && i + 1 < args.length) {
                ...
                environment = args[++i];
            } else if (("--refundId".equals(args[i]) || "-r".equals(args[i])) && i + 1 < args.length) {
                refundId = args[++i];
            }
        }

        // Validate required refundId
        if (refundId == null) {
            System.out.println("❌ ERROR: refundId is required!");
            System.out.println("Usage: mvn exec:java \"-Dexec.mainClass=com.katorymnd.pawapay.examples.direct.TestRefundIDCheck\" \"-Dexec.args=-r <refundId> [-v v1|v2] [-e sandbox|production]\"");
            System.exit(1);
        }

        System.out.println("🔍 Testing PawaPay Refund Status Check API " + apiVersion.toUpperCase() + "...");
        ...
        try {
            Map<String, Object> result = checkRefundStatus(environment, apiVersion, refundId);
