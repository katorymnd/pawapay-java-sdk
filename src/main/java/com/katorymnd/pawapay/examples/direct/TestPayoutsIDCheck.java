...
/**
 * Java SDK for PawaPay API - Direct payout status check
 */
public class TestPayoutsIDCheck {

    // Configuration
    private static final Map<String, Map<String, String>> CONFIG = new HashMap<>() {{
        put("sandbox", new HashMap<>() {{
            put("api_url", "https://api.sandbox.pawapay.io/payouts/");
        ...
        put("production", new HashMap<>() {{
            put("api_url", "https://api.pawapay.io/payouts/");
    ...
    /**
     * Extracts base URL (scheme + host) from a full URL
     */
    ...
    public static Map<String, Object> checkPayoutStatus(
            String environment,
            String apiVersion,
            String payoutId) throws IOException {
        ...
        // 2. Validate Payout ID (Strict UUID v4 check)
        if (!isValidUuidV4(payoutId)) {
            throw new IllegalArgumentException("payoutId must be a valid UUID v4.");
        }

        String rawConfigUrl = CONFIG.get(environment).get("api_url");
        String apiToken = CONFIG.get(environment).get("api_token");

        // 3. Normalize base URL (extract origin to avoid double pathing)
        String apiBaseUrl;
        try {
            apiBaseUrl = extractBaseUrl(rawConfigUrl);
        } catch (MalformedURLException e) {
            throw new IllegalArgumentException("Invalid API URL in configuration: " + rawConfigUrl);
        }

        // 4. Build endpoint per version
        String endpoint;
        if ("v2".equals(apiVersion)) {
            endpoint = "/v2/payouts/" + payoutId;
        } else {
            endpoint = "/payouts/" + payoutId;
        }
        ...
        try (Response response = client.newCall(request).execute()) {
            ...
            if (response.code() == 200) {
                ...
                result.put("message", "Payout status retrieved successfully!");
                ...
                error.put("message", "Failed to retrieve payout status. HTTP Status: " + response.code());
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            if (("--version".equals(args[i]) || "-v".equals(args[i])) && i + 1 < args.length) {
                ...
                environment = args[++i];
            } else if (("--payoutId".equals(args[i]) || "-p".equals(args[i])) && i + 1 < args.length) {
                payoutId = args[++i];
            }
        }

        // Validate required payoutId
        if (payoutId == null) {
            System.out.println("❌ ERROR: payoutId is required!");
            System.out.println("Usage: mvn exec:java \"-Dexec.mainClass=com.katorymnd.pawapay.examples.direct.TestPayoutsIDCheck\" \"-Dexec.args=-p <payoutId> [-v v1|v2] [-e sandbox|production]\"");
            System.exit(1);
        }

        System.out.println("🔍 Testing PawaPay Payout Status Check API " + apiVersion.toUpperCase() + "...");
        ...
        try {
            Map<String, Object> result = checkPayoutStatus(environment, apiVersion, payoutId);
